...
const RemoveStudentModal = ({ classId, onClose, onRemoveSuccess }) => {
  ...
  const fetchStudents = async () => {
    setLoading(true);
    try {
      const data = await userService.getStudentsInClass(classId);
  ...
  const handleRemoveStudent = async () => {
    ...
    setLoading(true);
    try {
      const result = await classroomService.removeStudents(classId, selectedStudentIds);
      ...
      if (result.status === 200) {
        ...
        toast.error(
          `Xóa học sinh thất bại: ${result.message || "Không rõ nguyên nhân"}`
        );
      }
    } catch (error) {
