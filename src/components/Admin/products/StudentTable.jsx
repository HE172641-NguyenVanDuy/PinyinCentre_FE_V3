...
const StudentTable = () => {
  ...
  // Gọi API để lấy danh sách học viên theo vai trò và trạng thái
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await userService.getUsersByRole("STUDENT", statusFilter, 1, 1000);
      
      if (data.status === 200 && Array.isArray(data.result)) {
        setStudents(data.result);
      } else {
        toast.error(data.message || "Lấy dữ liệu học viên thất bại");
      }
    } catch (error) {
